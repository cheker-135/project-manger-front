...
export const login = (email, password) => async (dispatch) => {
  try {
    ...
    // Store isAdmin status along with token in local storage
    localStorage.setItem('userInfo', JSON.stringify({ token: userInfo.token, isAdmin: userInfo.isAdmin , isAgent: userInfo.isAgent }));
    ...
    // Dispatch socket connection success and login success actions
    socket.on('connect', () => {
      dispatch({ type: SOCKET_CONNECT_SUCCESS, payload: socket });
      dispatch({ type: USER_LOGIN_SUCCESS, payload: { userInfo, socket } });
      // Dispatch action to fetch notifications
     dispatch(getNotifications(userInfo.token));
      socket.emit('join-notifications', { room: userInfo._id });
    });
...
export const fetchUsers = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get('/api/users/all', config); // Adjust API endpoint accordingly
...
export const getUserData = (token) => async (dispatch) => {
  try {
    ...
    socket.on('connect', () => {
      dispatch({ type: SOCKET_CONNECT_SUCCESS, payload: socket });
      dispatch({ type: USER_LOGIN_SUCCESS, payload: { userInfo: data, socket } });
